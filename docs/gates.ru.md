...
Локальное regex-сканирование `name`, `description` и `inputSchema` каждого инструмента. 26 правил в
наборе **v5**: 15 `block`, 11 `advise`, и у 15 из них есть контекстный **guard** — именованная
...
**v5: текст приводится к обычному виду до того, как его читает любое правило.** NFKC превращает
широкие буквы, лигатуры и другие совместимые формы в обычные; невидимые символы внутри слова
выбрасываются; блок Unicode-тегов (невидимые копии ASCII, в которых можно спрятать целое предложение)
расшифровывается; внутри слова, где латиница смешана с кириллицей или греческим, буквы-двойники
заменяются латинскими, а слово целиком на одном алфавите не трогается. Английское правило больше не
обходится широкими буквами, пробелом нулевой ширины, тегами или кириллической `о` — на каком бы языке
ни был окружающий текст. Идентификатор свёртки публикуется как `fold` и входит в дайджест. Два правила
скрытых нагрузок читают **сырой** текст (`raw: true`): они ищут именно то, что свёртка убирает.

Смысл регэксп-таблица не читает: формулировка на языке, для которого правила не написаны, вне её.
v5 добавляет то, что от языка не зависит: свёртку, пару `TOOL_DEF_SECRET_EXFIL` (хранилище секретов и
внешний адрес в пределах 100 символов; только уведомляет — её единственное срабатывание на 10 645
живых серверах оказалось честным), блок тегов и изоляты направления в `TOOL_DEF_HIDDEN_UNICODE`, а
HISTOR показывает каждый внешний адрес, впервые появившийся в определениях сервера. Распознавание по
смыслу — задача классификатора, а не этой таблицы.

Ещё v5 убирает три измеренных ложных срабатывания: «send the user to https://…» (переход человека,
guard `navigation`), «keep calling … without asking the user» (самостоятельная работа, guard
`autonomy`) и соединитель нулевой ширины внутри эмодзи. На корпусе из 10 645 серверов v5 блокирует 56
там, где v4 блокировал 63, и не блокирует ни одного, которого не блокировал v4.

Каждое правило объявляет, на каких из этих трёх **поверхностей** оно работает; 17 из 26 включают
